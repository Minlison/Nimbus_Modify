...
- (id<QMUITableViewDelegate>)forwardingTo:(id<QMUITableViewDelegate>)forwardDelegate {
  [self.forwardDelegates addObject:forwardDelegate];
  return self;
}

- (void)removeForwarding:(id<QMUITableViewDelegate>)forwardDelegate {
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  for (id<QMUITableViewDelegate> delegate in self.forwardDelegates) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  // On iOS 8 and below, QMUITableViewDelegate is marked "assign". This means that the delegate
...
#pragma clang diagnostic pop
  ...
  // Forward the invocation along.
  for (id<QMUITableViewDelegate> delegate in strongSelf.forwardDelegates) {
    if ([delegate respondsToSelector:_cmd]) {
